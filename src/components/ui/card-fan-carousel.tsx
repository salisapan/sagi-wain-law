  ...
  return (
    <section className="relative z-20 flex w-full flex-col items-center px-4 py-4 md:px-8 lg:py-8">
      <div className="flex w-full max-w-[90rem] items-center justify-center">
        <div
          ref={containerRef}
          className="fan-layout relative flex h-64 w-full max-w-[80rem] items-center justify-center sm:h-72 md:h-80 lg:h-[26rem] xl:h-[30rem]"
        >
          {cards.map((card, index) => {
            const Icon = card.platform ? PlatformIcon[card.platform] : null
            const image = (
              <div className="relative h-full w-full overflow-hidden rounded-2xl border border-white/10 shadow-2xl shadow-black/40">
                <img src={card.imgUrl} loading="lazy" alt={card.alt || `Card ${index}`} className="absolute inset-0 h-full w-full object-cover" />
                {(card.caption || Icon) && (
                  <>
                    <div className="absolute inset-0 bg-gradient-to-t from-navy via-navy/10 to-transparent" />
                    {Icon && (
                      <div className="absolute end-2 top-2 flex h-7 w-7 items-center justify-center rounded-full bg-gold-metallic bg-[length:200%_auto] shadow-gold">
                        <Icon className="h-3.5 w-3.5 text-navy" />
                      </div>
                    )}
                    <div className="absolute inset-x-0 bottom-0 flex flex-col gap-0.5 p-3">
                      <Play className="h-4 w-4 text-white/80" fill="white" strokeWidth={0} />
                      {card.caption && <p className="text-[11px] font-medium leading-snug text-white">{card.caption}</p>}
                      {card.views && <p className="text-[10px] text-gold-light">{card.views}</p>}
                    </div>
                  </>
                )}
              </div>
            ...
            const cardClassName =
              'fan-card absolute top-0 h-full w-[clamp(6.5rem,22vw,15rem)] ms-[calc(clamp(6.5rem,22vw,15rem)/-2)] start-1/2'
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith('http') ? '_blank' : '_self'} rel="noopener noreferrer" className={`${cardClassName} block cursor-pointer`}>
                {image}
              </a>
            ) : (
      ...
      {needsPagination && (
        <div className="z-30 mt-4 flex items-center justify-center gap-4 md:mt-6">
          <button className={`${ARROW_CLASSES} h-10 w-10 md:h-12 md:w-12`} onClick={() => cycle('left')} aria-label="הקודם">
            {chevron('left')}
          </button>
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`h-2 w-2 rounded-full transition-all duration-300 ${i === centerIndex ? 'scale-[1.3] bg-gold-light' : 'bg-white/15'}`} />
            ))}
          </div>
          <button className={`${ARROW_CLASSES} h-10 w-10 md:h-12 md:w-12`} onClick={() => cycle('right')} aria-label="הבא">
            {chevron('right')}
          </button>
        </div>
      )}
